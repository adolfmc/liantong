...
public class MultiPointOutputStream {
    ...
    public synchronized void cancel() {
        ...
        if (this.allNoSyncLength.get() > 0) {
            ...
            for (Integer num : this.requireStreamBlocks) {
                try {
                    ...
                    Util.m13741d(TAG, "OutputStream close failed task[" + this.task.getId() + "] block[" + num + "]" + e);
                }
            }
        ...
        for (Integer num2 : this.requireStreamBlocks) {
            try {
                ...
                Util.m13741d(TAG, "OutputStream close failed task[" + this.task.getId() + "] block[" + num2 + "]" + e2);
            }
        }
        this.store.onTaskEnd(this.task.getId(), EndCause.CANCELED, null);
        return;
    ...
    public void done(int i) throws IOException {
        ...
        try {
            ...
            if (this.syncFuture != null && !this.syncFuture.isDone()) {
                ...
                Util.m13741d(TAG, "OutputStream done but no need to ensure sync, because the sync job not run yet. task[" + this.task.getId() + "] block[" + i + "]");
            } else {
                Util.m13741d(TAG, "OutputStream done but no need to ensure sync, because the syncFuture.isDone[" + this.syncFuture.isDone() + "] task[" + this.task.getId() + "] block[" + i + "]");
            }
        } finally {
            close(i);
        }
    ...
    synchronized void close(int i) throws IOException {
        ...
        if (downloadOutputStream != null) {
            ...
            Util.m13741d(TAG, "OutputStream close task[" + this.task.getId() + "] block[" + i + "]");
        }
    }
    ...
    void inspectStreamState(StreamsState streamsState) {
        ...
        if (size != this.requireStreamBlocks.size()) {
            Util.m13741d(TAG, "task[" + this.task.getId() + "] current need fetching block count " + this.requireStreamBlocks.size() + " is not equal to no more stream block count " + size);
            streamsState.isNoMoreStream = false;
        } else {
            Util.m13741d(TAG, "task[" + this.task.getId() + "] current need fetching block count " + this.requireStreamBlocks.size() + " is equal to no more stream block count " + size);
    ...
    void runSyncDelayException() {
        try {
            ...
            Util.m13738w(TAG, "Sync to breakpoint-store for task[" + this.task.getId() + "] failed with cause: " + e);
        }
    }

    void runSync() throws IOException {
        Util.m13741d(TAG, "OutputStream start flush looper task[" + this.task.getId() + "] with syncBufferIntervalMills[" + this.syncBufferIntervalMills + "] syncBufferSize[" + this.syncBufferSize + "]");
        ...
        while (true) {
            ...
            if (this.state.isStreamsEndOrChanged()) {
                Util.m13741d(TAG, "runSync state change isNoMoreStream[" + this.state.isNoMoreStream + "] newNoMoreStreamBlockList[" + this.state.newNoMoreStreamBlockList + "]");
                if (this.allNoSyncLength.get() > 0) {
                    flushProcess();
                }
                ...
                j = this.syncBufferIntervalMills;
            } else {
        ...
        for (int i = 0; i < size; i++) {
            Thread valueAt = this.parkedRunBlockThreadMap.valueAt(i);
            if (valueAt != null) {
                unparkThread(valueAt);
            }
        }
        this.parkedRunBlockThreadMap.clear();
        Util.m13741d(TAG, "OutputStream stop flush looper task[" + this.task.getId() + "]");
    }
    ...
    */
    void flushProcess() throws java.io.IOException {
        /*
            ...
            monitor-enter(r0)
            android.util.SparseArray<java.util.concurrent.atomic.AtomicLong> r1 = r11.noSyncLengthMap     // Catch: java.lang.Throwable -> Le9
        ...
        L40:
            ...
            com.liulishuo.okdownload.core.Util.m13738w(r3, r1)
            r1 = r2
            goto L5a
        L59:
            r1 = 1
        ...
        L60:
            ...
            com.liulishuo.okdownload.core.Util.m13741d(r8, r3)
            int r2 = r2 + 1
            goto L60
        Ld9:
        ...
        if (downloadOutputStream == null) {
            ...
            if (isUriFileScheme) {
                ...
                if (file.createNewFile()) {
                    Util.m13741d(TAG, "Create new file: " + file.getName());
                }
            ...
            if (this.supportSeek) {
                ...
                if (rangeLeft > 0) {
                    ...
                    Util.m13741d(TAG, "Create output stream write from (" + this.task.getId() + ") block(" + i + ") " + rangeLeft);
                }
            }
