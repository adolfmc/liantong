...
public class DownloadUrlConnection implements DownloadConnection, DownloadConnection.Connected {
    ...
    void configUrlConnection() throws IOException {
        Util.m13741d(TAG, "config connection for " + this.url);
        Configuration configuration = this.configuration;
        if (configuration == null || configuration.proxy == null) {
            this.connection = NBSInstrumentation.openConnection(this.url.openConnection());
        } else {
            this.connection = NBSInstrumentation.openConnectionWithProxy(this.url.openConnection(this.configuration.proxy));
        }
