...
public class BreakpointInterceptor implements Interceptor.Connect, Interceptor.Fetch {
    ...
    public DownloadConnection.Connected interceptConnect(DownloadChain downloadChain) throws IOException {
        ...
        if (info.getBlockCount() == 1 && !info.isChunked()) {
            ...
            if (exactContentLengthRangeFrom0 > 0 && exactContentLengthRangeFrom0 != totalLength) {
                Util.m13741d(TAG, "SingleBlock special check: the response instance-length[" + exactContentLengthRangeFrom0 + "] isn't equal to the instance length from trial-connection[" + totalLength + "]");
                ...
                if (z) {
                    Util.m13738w(TAG, "Discard breakpoint because of on this special case, we have to download from beginning");
    ...
    */
    long getExactContentLengthRangeFrom0(@android.support.annotation.NonNull com.liulishuo.okdownload.core.connection.DownloadConnection.Connected r7) {
        /*
            r6 = this;
