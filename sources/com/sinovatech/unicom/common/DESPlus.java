...
public class DESPlus {
    ...
    public DESPlus() {
        this.strKey = strDefaultKey;
        this.f18447C = (byte[][]) Array.newInstance(byte.class, 17, 28);
        this.f18448D = (byte[][]) Array.newInstance(byte.class, 17, 28);
        this.f18449K = (byte[][]) Array.newInstance(byte.class, 17, 48);
    }
    ...
    public DESPlus(String str) {
        ...
        this.f18448D = (byte[][]) Array.newInstance(byte.class, 17, 28);
        this.f18449K = (byte[][]) Array.newInstance(byte.class, 17, 48);
        this.strKey = String.valueOf(str + strDefaultKey).substring(0, 8);
    }
    ...
    private void Fsetkeystar(byte[] bArr) {
        int i = 0;
        for (int i2 = 0; i2 < 28; i2++) {
            this.f18447C[0][i2] = bArr[pc_1_cp[i2] - 1];
        }
        for (int i3 = 0; i3 < 28; i3++) {
            this.f18448D[0][i3] = bArr[pc_1_dp[i3] - 1];
        }
        while (i < 16) {
            byte[][] bArr2 = this.f18447C;
            int i4 = i + 1;
            FLS(bArr2[i], bArr2[i4], ls_countp[i]);
            byte[][] bArr3 = this.f18448D;
            FLS(bArr3[i], bArr3[i4], ls_countp[i]);
            Fson(this.f18447C[i4], this.f18448D[i4], this.f18449K[i4]);
            i = i4;
        }
    }
    ...
    private void Fs_box(byte[] bArr, byte[] bArr2) {
        ...
        while (i < 8) {
            ...
            int i6 = 0;
            while (i6 < 4) {
                bArr2[i5] = iu2b((b2iu(bArr3[i]) >>> i4) & 1);
                i4--;
                i6++;
                i5++;
            }
    ...
    /* renamed from: FF */
    private void m8001FF(int i, byte[] bArr, byte[] bArr2, byte[] bArr3, byte[] bArr4) {
        byte[] bArr5 = new byte[64];
        ...
        for (int i3 = 0; i3 < 48; i3++) {
            bArr5[i3] = iu2b((b2iu(bArr5[i3]) + b2iu(this.f18449K[i][i3])) & 1);
        }
        Fs_box(bArr5, bArr6);
    ...
    void Fcompress0(byte[] bArr, byte[] bArr2) {
        for (int i = 0; i < 8; i++) {
            bArr2[i] = 0;
            int i2 = 7;
            for (int i3 = 0; i3 < 8; i3++) {
    ...
    private void Fencrypt0(byte[] bArr, byte[] bArr2) {
        ...
        for (int i = 1; i < 17; i++) {
            m8001FF(i, bArr3, bArr4, bArr5, bArr6);
            for (int i2 = 0; i2 < 32; i2++) {
                bArr3[i2] = bArr5[i2];
                bArr4[i2] = bArr6[i2];
            }
    ...
    public static void main(String[] strArr) {
        DESPlus dESPlus = new DESPlus("y1h2x3x4");
        String Encode = dESPlus.Encode("1107804875904111869");
        PrintStream printStream = System.out;
        printStream.println("加密后:[" + Encode + "]");
        String trim = dESPlus.Decode("45A25BA6A31EFEB98ECE5E564ABAE3F4B5B80E7F702FEF89").toString().trim();
        PrintStream printStream2 = System.out;
        printStream2.println("解密后:[" + trim + "]");
    }
    ...
    public String Encode(String str) {
        ...
        byte[] bytes = str.getBytes();
        String str2 = "";
        for (int i = 0; i < ENCRYPT(this.strKey.getBytes(), bytes, new byte[3000], bytes.length); i++) {
            str2 = str2 + byteHEX(bArr[i]);
        }
        return str2;
    }
    ...
    public String Decode(String str) {
        ...
        return new String(bArr, 0, DECRYPT(this.strKey.getBytes(), bArr, hexStr2ByteArr, hexStr2ByteArr.length));
    }
}
