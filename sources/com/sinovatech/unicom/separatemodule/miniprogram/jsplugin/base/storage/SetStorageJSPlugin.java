...
@Route(path = "/MsJSPlugin/setStorage")
@NBSInstrumented
/* loaded from: E:\11480076_dexfile_execute.dex.fixout.dex */
public class SetStorageJSPlugin extends StorageJSPlugin {
    ...
    private JSONObject setStorage() throws Exception {
        ...
        try {
            ...
            if (((!(jSONObject3 instanceof JSONObject) ? jSONObject3.toString() : NBSJSONObjectInstrumentation.toString(jSONObject3)).getBytes().length / 1024) + JSStorageBox.querySizeForAppId(getStoragePrefixId()) > 10240) {
                jSONObject.put("msg", "允许缓存的最大数据量不能超过10M");
                return jSONObject;
            }
            String storagePrefixId = getStoragePrefixId();
            JSONObject jSONObject4 = this.parameterJO;
            JSStorageBox.put(storagePrefixId, string, !(jSONObject4 instanceof JSONObject) ? jSONObject4.toString() : NBSJSONObjectInstrumentation.toString(jSONObject4));
            this.isSuccess = true;
            return jSONObject;
        } catch (Exception e) {
            e.printStackTrace();
