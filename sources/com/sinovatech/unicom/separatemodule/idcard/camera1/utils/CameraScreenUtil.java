...
import com.sinovatech.unicom.separatemodule.audience.util.PhoneRomUtil;
 ...
 */
    ...
    public static void finish(final Activity activity) {
        ...
        if (!PhoneRomUtil.isSmartisanRom()) {
            ...
            SafeHandler.getInst().postDelayed(new Runnable() { // from class: com.sinovatech.unicom.separatemodule.idcard.camera1.utils.CameraScreenUtil.1
        ...
        if (screenHeight != getRealScreenHeight()) {
            ...
            if ((d <= 0.5d && f / realScreenHeight2 > 0.5d) || (d > 0.5d && f / realScreenHeight2 <= 0.5d)) {
                ...
                if (viewRect == null || viewRect.width() == 0 || viewRect.height() == 0) {
                    ...
                    SafeHandler.getInst().post(new Runnable() { // from class: com.sinovatech.unicom.separatemodule.idcard.camera1.utils.CameraScreenUtil.2
                        @Override // java.lang.Runnable
                        public void run() {
                            CameraScreenUtil.getDisplayRate((Activity) weakReference.get(), onDisplayRectListener);
                        }
                    });
                    return;
                }
