 ...
 */
    ...
    public OverlayView(Context context, AttributeSet attributeSet, int i) {
        ...
        this.mTouchPointThreshold = getResources().getDimensionPixelSize(C11657R.dimen.ucrop_default_crop_rect_corner_touch_threshold);
        this.mCropRectMinSize = getResources().getDimensionPixelSize(C11657R.dimen.ucrop_default_crop_rect_min_size);
        this.mCropRectCornerTouchAreaLineLength = getResources().getDimensionPixelSize(C11657R.dimen.ucrop_default_crop_rect_corner_touch_area_line_length);
        init();
    }
    ...
    protected void drawCropGrid(@NonNull Canvas canvas) {
        ...
        if (this.mShowCropGrid) {
            if (this.mGridPoints == null && !this.mCropViewRect.isEmpty()) {
                ...
                while (i3 < this.mCropGridRowCount) {
                    ...
                    this.mGridPoints[i7] = (this.mCropViewRect.height() * (f / (this.mCropGridRowCount + 1))) + this.mCropViewRect.top;
                    i3++;
                    i4 = i7 + 1;
                }
                for (int i8 = 0; i8 < this.mCropGridColumnCount; i8++) {
                    int i9 = i4 + 1;
                    float f2 = i8 + 1.0f;
                    this.mGridPoints[i4] = (this.mCropViewRect.width() * (f2 / (this.mCropGridColumnCount + 1))) + this.mCropViewRect.left;
                    int i10 = i9 + 1;
                    this.mGridPoints[i9] = this.mCropViewRect.top;
                    int i11 = i10 + 1;
                    this.mGridPoints[i10] = (this.mCropViewRect.width() * (f2 / (this.mCropGridColumnCount + 1))) + this.mCropViewRect.left;
                    i4 = i11 + 1;
        ...
        if (this.mFreestyleCropMode != 0) {
            canvas.save();
            this.mTempRect.set(this.mCropViewRect);
            this.mTempRect.inset(this.mCropRectCornerTouchAreaLineLength, -i);
            canvas.clipRect(this.mTempRect, Region.Op.DIFFERENCE);
            this.mTempRect.set(this.mCropViewRect);
            this.mTempRect.inset(-i2, this.mCropRectCornerTouchAreaLineLength);
            canvas.clipRect(this.mTempRect, Region.Op.DIFFERENCE);
    ...
    public void processStyledAttributes(@NonNull TypedArray typedArray) {
        this.mCircleDimmedLayer = typedArray.getBoolean(C11657R.styleable.ucrop_UCropView_ucrop_circle_dimmed_layer, false);
        this.mDimmedColor = typedArray.getColor(C11657R.styleable.ucrop_UCropView_ucrop_dimmed_color, getResources().getColor(C11657R.C11658color.ucrop_color_default_dimmed));
        ...
        this.mShowCropFrame = typedArray.getBoolean(C11657R.styleable.ucrop_UCropView_ucrop_show_frame, true);
        initCropGridStyle(typedArray);
        this.mShowCropGrid = typedArray.getBoolean(C11657R.styleable.ucrop_UCropView_ucrop_show_grid, true);
    }

    private void initCropFrameStyle(@NonNull TypedArray typedArray) {
        int dimensionPixelSize = typedArray.getDimensionPixelSize(C11657R.styleable.ucrop_UCropView_ucrop_frame_stroke_size, getResources().getDimensionPixelSize(C11657R.dimen.ucrop_default_crop_frame_stoke_width));
        int color = typedArray.getColor(C11657R.styleable.ucrop_UCropView_ucrop_frame_color, getResources().getColor(C11657R.C11658color.ucrop_color_default_crop_frame));
    ...
    private void initCropGridStyle(@NonNull TypedArray typedArray) {
        int dimensionPixelSize = typedArray.getDimensionPixelSize(C11657R.styleable.ucrop_UCropView_ucrop_grid_stroke_size, getResources().getDimensionPixelSize(C11657R.dimen.ucrop_default_crop_grid_stoke_width));
        int color = typedArray.getColor(C11657R.styleable.ucrop_UCropView_ucrop_grid_color, getResources().getColor(C11657R.C11658color.ucrop_color_default_crop_grid));
        this.mCropGridPaint.setStrokeWidth(dimensionPixelSize);
        this.mCropGridPaint.setColor(color);
        this.mCropGridRowCount = typedArray.getInt(C11657R.styleable.ucrop_UCropView_ucrop_grid_row_count, 2);
        this.mCropGridColumnCount = typedArray.getInt(C11657R.styleable.ucrop_UCropView_ucrop_grid_column_count, 2);
    }
}
