...
public abstract class HuaweiMobileServicesUtil {
    ...
    public static int isHuaweiMobileServicesAvailable(Context context, int i) {
        ...
        if (HMSPackageManager.getInstance(context).isUseOldCertificate()) {
            HMSLog.m14112e("HuaweiMobileServicesUtil", "The CP uses the old certificate to terminate the connection.");
            return 13;
        }
        ...
        HMSLog.m14110i("HuaweiMobileServicesUtil", "hmsPackageName is " + hMSPackageNameForMultiService);
        ...
        if (PackageManagerHelper.PackageStates.NOT_INSTALLED.equals(hMSPackageStatesForMultiService)) {
            HMSLog.m14110i("HuaweiMobileServicesUtil", "HMS is not installed");
            return 1;
        }
        ...
        if (HwBuildEx.VERSION.EMUI_SDK_INT < 5 && packageManagerHelper.getPackageVersionCode(hMSPackageNameForMultiService) < 20500000 && ReadApkFileUtil.isCertFound(hmsPath)) {
            ...
            HMSLog.m14110i("HuaweiMobileServicesUtil", "HMS is spoofed");
            return 9;
        } else if (PackageManagerHelper.PackageStates.DISABLED.equals(hMSPackageStatesForMultiService)) {
            HMSLog.m14110i("HuaweiMobileServicesUtil", "HMS is disabled");
            return 3;
        } else {
            if (PackageManagerHelper.PackageStates.NOT_INSTALLED.equals(hMSPackageStatesForMultiService)) {
            ...
            HMSLog.m14110i("HuaweiMobileServicesUtil", "connect versionCode:" + hmsMultiServiceVersion);
            if (HMSPackageManager.getInstance(context).isApkUpdateNecessary(i)) {
                HMSLog.m14110i("HuaweiMobileServicesUtil", "The current version does not meet the minimum version requirements");
                return 2;
            }
            return 0;
        }
