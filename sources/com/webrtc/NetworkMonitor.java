 ...
 */
    ...
    private NetworkMonitor() {
        this.networkChangeDetectorFactory = new NetworkChangeDetectorFactory() { // from class: com.webrtc.NetworkMonitor.1
    ...
    private void startMonitoring(@Nullable Context context, long j) {
        Logging.m5305d(TAG, "Start monitoring with native observer " + j);
        if (context == null) {
    ...
    private void stopMonitoring(long j) {
        Logging.m5305d(TAG, "Stop monitoring with native observer " + j);
        stopMonitoring();
        synchronized (this.nativeNetworkObservers) {
    ...
    private NetworkChangeDetector createNetworkChangeDetector(Context context) {
        return this.networkChangeDetectorFactory.create(new NetworkChangeDetector.Observer() { // from class: com.webrtc.NetworkMonitor.2
    ...
    private void updateObserverActiveNetworkList(long j) {
        ...
        synchronized (this.networkChangeDetectorLock) {
            activeNetworkList = this.networkChangeDetector == null ? null : this.networkChangeDetector.getActiveNetworkList();
        }
