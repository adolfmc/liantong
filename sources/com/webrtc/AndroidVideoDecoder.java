 ...
 */
    ...
    public AndroidVideoDecoder(MediaCodecWrapperFactory mediaCodecWrapperFactory, String str, VideoCodecMimeType videoCodecMimeType, int i, @Nullable EglBase.Context context) {
        ...
        Logging.m5305d(TAG, "ctor name: " + str + " type: " + videoCodecMimeType + " color format: " + i + " context: " + context);
    ...
    private VideoCodecStatus initDecodeInternal(int i, int i2) {
        ...
        Logging.m5305d(TAG, "initDecodeInternal name: " + this.codecName + " type: " + this.codecType + " width: " + i + " height: " + i2);
        if (this.outputThread != null) {
            Logging.m5304e(TAG, "initDecodeInternal called while the codec is already running");
        ...
        RtcLogCapturer.reportLog(RtcLogEvent.INIT_DECODER_START, "AVDecoder-initDecodeInternal", this.codecName, this.codecType.mimeType(), Integer.valueOf(i), Integer.valueOf(i2));
        try {
            ...
            try {
                ...
                RtcLogCapturer.reportLog(RtcLogEvent.INIT_DECODER_SUCCESS, "AVDecoder-initDecodeInternal");
                Logging.m5305d(TAG, "initDecodeInternal done");
                return VideoCodecStatus.OK;
            } catch (IllegalArgumentException | IllegalStateException e) {
                Logging.m5303e(TAG, "initDecode failed", e);
                release();
                RtcLogEvent rtcLogEvent = RtcLogEvent.INIT_DECODER_FAILED;
                RtcLogCapturer.reportLog(rtcLogEvent, "AVDecoder-initDecodeInternal", "initDecode failed " + RtcLogCapturer.getErrorInfo(e.getMessage()));
                return VideoCodecStatus.FALLBACK_SOFTWARE;
            }
        } catch (IOException | IllegalArgumentException | IllegalStateException unused) {
            Logging.m5304e(TAG, "Cannot create media decoder " + this.codecName);
            return VideoCodecStatus.FALLBACK_SOFTWARE;
        }
    }
    ...
    public void foundSei(ByteBuffer byteBuffer) {
        if (byteBuffer != null) {
            Logging.m5305d(TAG, "found sei frame size " + byteBuffer.capacity());
            ByteBuffer allocate = ByteBuffer.allocate(byteBuffer.capacity());
            byteBuffer.rewind();
            allocate.put(byteBuffer);
            byteBuffer.rewind();
            allocate.flip();
            VideoDecoder.VideoDecoderCallback videoDecoderCallback = this.mDecoderCallback;
    ...
    public VideoCodecStatus decode(EncodedImage encodedImage, VideoDecoder.DecodeInfo decodeInfo) {
        ...
        if (this.codec == null || this.callback == null) {
            ...
            Logging.m5304e(TAG, "decode() - no input data");
            return VideoCodecStatus.ERR_PARAMETER;
        } else {
            int remaining = encodedImage.buffer.remaining();
            if (remaining == 0) {
                Logging.m5304e(TAG, "decode() - input buffer empty");
                return VideoCodecStatus.ERR_PARAMETER;
            }
            ...
            if (encodedImage.encodedWidth * encodedImage.encodedHeight <= 0 || ((encodedImage.encodedWidth == i && encodedImage.encodedHeight == i2) || (reinitDecode = reinitDecode(encodedImage.encodedWidth, encodedImage.encodedHeight)) == VideoCodecStatus.OK)) {
                if (this.keyFrameRequired && encodedImage.frameType != EncodedImage.FrameType.VideoFrameKey) {
                    Logging.m5304e(TAG, "decode() - key frame required first");
                    return VideoCodecStatus.NO_OUTPUT;
                }
                try {
                    int dequeueInputBuffer = this.codec.dequeueInputBuffer(500000L);
                    if (dequeueInputBuffer < 0) {
                        Logging.m5304e(TAG, "decode() - no HW buffers available; decoder falling behind");
                        return VideoCodecStatus.ERROR;
                    }
                    try {
                        ByteBuffer byteBuffer = this.codec.getInputBuffers()[dequeueInputBuffer];
                        if (byteBuffer.capacity() < remaining) {
                            Logging.m5304e(TAG, "decode() - HW buffer too small");
                            return VideoCodecStatus.ERROR;
                        }
                    ...
                    RtcLogEvent rtcLogEvent = RtcLogEvent.DECODER_ERROR;
                    RtcLogCapturer.reportLog(rtcLogEvent, "AVDecoder-decode", -1, "dequeueInputBuffer failed " + RtcLogCapturer.getErrorInfo(e3.getMessage()));
                    return VideoCodecStatus.ERROR;
                }
            }
            return reinitDecode;
    ...
    /* JADX WARN: Type inference failed for: r1v0, types: [java.lang.Thread, com.webrtc.MediaCodecWrapper] */
    private VideoCodecStatus releaseInternal() {
        if (!this.running) {
            Logging.m5305d(TAG, "release: Decoder is not running.");
            return VideoCodecStatus.OK;
        }
        try {
            this.running = false;
            if (!ThreadUtils.joinUninterruptibly(this.outputThread, 5000L)) {
                Logging.m5303e(TAG, "Media decoder release timeout", new RuntimeException());
                return VideoCodecStatus.TIMEOUT;
            } else if (this.shutdownException != null) {
                Logging.m5303e(TAG, "Media decoder release error", new RuntimeException(this.shutdownException));
    ...
    private Thread createOutputThread() {
        return new Thread("AndroidVideoDecoder.outputThread") { // from class: com.webrtc.AndroidVideoDecoder.1
    ...
    protected void deliverDecodedFrame() {
        ...
        try {
            ...
            if (dequeueOutputBuffer == -2) {
                ...
                Logging.m5302v(TAG, "dequeueOutputBuffer returned " + dequeueOutputBuffer);
            } else {
                ...
                if (!this.hasDecodedFirstFrame) {
                    RtcLogCapturer.reportLog(RtcLogEvent.DECODER_FIRST_FRAME, "AVDecoder-deliverDecodedFrame");
                }
            ...
            Logging.m5303e(TAG, "deliverDecodedFrame failed", e);
        }
    }
    ...
    private void deliverByteFrame(int i, MediaCodec.BufferInfo bufferInfo, int i2, Integer num) {
        ...
        if (bufferInfo.size < ((i3 * i4) * 3) / 2) {
            Logging.m5304e(TAG, "Insufficient output buffer size: " + bufferInfo.size);
            return;
        }
    ...
    private void reformat(MediaFormat mediaFormat) {
        ...
        int integer2;
        this.outputThreadChecker.checkIsOnValidThread();
        Logging.m5305d(TAG, "Decoder format changed: " + mediaFormat.toString());
        ...
        synchronized (this.dimensionLock) {
            ...
            if (this.surfaceTextureHelper == null && mediaFormat.containsKey("color-format")) {
                ...
                Logging.m5305d(TAG, "Color: 0x" + Integer.toHexString(this.colorFormat));
            ...
            synchronized (this.dimensionLock) {
                ...
                Logging.m5305d(TAG, "Frame stride and slice height: " + this.stride + " x " + this.sliceHeight);
    ...
    public void releaseCodecOnOutputThread() {
        ...
        RtcLogCapturer.reportLog(RtcLogEvent.STOP_DECODER, "AVDecoder-releaseCodecOnOutputThread");
        try {
            this.codec.stop();
        } catch (Exception e) {
            Logging.m5303e(TAG, "Media decoder stop failed", e);
            RtcLogEvent rtcLogEvent = RtcLogEvent.STOP_DECODER_FAILED;
            RtcLogCapturer.reportLog(rtcLogEvent, "AVDecoder-releaseCodecOnOutputThread", -1, "Media decoder stop failed " + RtcLogCapturer.getErrorInfo(e.getMessage()));
        }
        try {
            this.codec.release();
        } catch (Exception e2) {
            Logging.m5303e(TAG, "Media decoder release failed", e2);
