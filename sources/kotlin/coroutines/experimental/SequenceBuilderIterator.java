...
final class SequenceBuilderIterator<T> extends SequenceBuilder<T> implements Iterator<T>, Coroutines<Unit>, KMarkers {
    private Iterator<? extends T> nextIterator;
    @Nullable
    private Coroutines<? super Unit> nextStep;
    private T nextValue;
    private int state;
    ...
    @Nullable
    public final Coroutines<Unit> getNextStep() {
        return this.nextStep;
    }

    public final void setNextStep(@Nullable Coroutines<? super Unit> coroutines) {
        this.nextStep = coroutines;
    }
    ...
    public boolean hasNext() {
        while (true) {
            ...
            this.state = 5;
            Coroutines<? super Unit> coroutines = this.nextStep;
            if (coroutines == null) {
                Intrinsics.throwNpe();
            }
            this.nextStep = null;
            coroutines.resume(Unit.INSTANCE);
        }
    }
    ...
    @Nullable
    public Object yield(T t, @NotNull Coroutines<? super Unit> coroutines) {
        this.nextValue = t;
        this.state = 3;
        setNextStep(CoroutineIntrinsics.normalizeContinuation(coroutines));
        return IntrinsicsKt.getCOROUTINE_SUSPENDED();
    }

    @Override // kotlin.coroutines.experimental.SequenceBuilder
    @Nullable
    public Object yieldAll(@NotNull Iterator<? extends T> it, @NotNull Coroutines<? super Unit> coroutines) {
        if (it.hasNext()) {
