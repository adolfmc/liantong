                ...
                while (true) {
                    if (i9 >= i3) {
                        ...
                        if (isMatch3 == 0) {
                            ...
                            if (valueDelimiterMatcher != null) {
                                ...
                                while (true) {
                                    if (i12 >= charArray.length) {
                                        strMatcher = variablePrefixMatcher;
                                        break;
                                    } else if (!isEnableSubstitutionInVariables && variablePrefixMatcher.isMatch(charArray, i12, i12, charArray.length) != 0) {
                                        strMatcher = variablePrefixMatcher;
                                        break;
                                    } else {
                            ...
                            list2.add(str2);
                            String resolveVariable = resolveVariable(str2, strBuilder, i6, i11);
                            if (resolveVariable == null) {
                                resolveVariable = str;
                            }
                            if (resolveVariable != null) {
                                int length = resolveVariable.length();
                                strBuilder.replace(i6, i11, resolveVariable);
                                int substitute = (substitute(strBuilder, i6, length, list2) + length) - (i11 - i6);
                                i11 += substitute;
                                i3 += substitute;
                                i4 += substitute;
                                cArr = strBuilder.buffer;
