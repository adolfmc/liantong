...
public class HMacSP800DRBG implements SP80090DRBG {
    ...
    /* renamed from: _K */
    private byte[] f26851_K;

    /* renamed from: _V */
    private byte[] f26852_V;
    ...
    public HMacSP800DRBG(Mac mac, int i, EntropySource entropySource, byte[] bArr, byte[] bArr2) {
        ...
        this.f26851_K = new byte[mac.getMacSize()];
        this.f26852_V = new byte[this.f26851_K.length];
        Arrays.fill(this.f26852_V, (byte) 1);
    ...
    private void hmac_DRBG_Update_Func(byte[] bArr, byte b) {
        this._hMac.init(new KeyParameter(this.f26851_K));
        Mac mac = this._hMac;
        byte[] bArr2 = this.f26852_V;
        mac.update(bArr2, 0, bArr2.length);
        this._hMac.update(b);
        if (bArr != null) {
            this._hMac.update(bArr, 0, bArr.length);
        }
        this._hMac.doFinal(this.f26851_K, 0);
        this._hMac.init(new KeyParameter(this.f26851_K));
        Mac mac2 = this._hMac;
        byte[] bArr3 = this.f26852_V;
        mac2.update(bArr3, 0, bArr3.length);
        this._hMac.doFinal(this.f26852_V, 0);
    }
