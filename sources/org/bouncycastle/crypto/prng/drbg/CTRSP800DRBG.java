...
public class CTRSP800DRBG implements SP80090DRBG {
    ...
    private void CTR_DRBG_Instantiate_algorithm(byte[] bArr, byte[] bArr2, byte[] bArr3) {
        ...
        this.f26845_V = new byte[blockSize];
        CTR_DRBG_Update(Block_Cipher_df, this._Key, this.f26845_V);
        this._reseedCounter = 1L;
    }

    private void CTR_DRBG_Reseed_algorithm(byte[] bArr) {
        CTR_DRBG_Update(Block_Cipher_df(Arrays.concatenate(getEntropy(), bArr), this._seedLength), this._Key, this.f26845_V);
        this._reseedCounter = 1L;
    }
    ...
    private int getMaxSecurityStrength(BlockCipher blockCipher, int i) {
        ...
        if (blockCipher.getAlgorithmName().equals(C0108a.f85c)) {
            return i;
        }
        return -1;
    }
    ...
    public int generate(byte[] bArr, byte[] bArr2, boolean z) {
        ...
        if (bArr2 != null) {
            ...
            CTR_DRBG_Update(bArr3, this._Key, this.f26845_V);
        } else {
            bArr3 = new byte[this._seedLength / 8];
        }
        byte[] bArr4 = new byte[this.f26845_V.length];
        this._engine.init(true, new KeyParameter(expandKey(this._Key)));
        for (int i = 0; i <= bArr.length / bArr4.length; i++) {
            int length = bArr.length - (bArr4.length * i) > bArr4.length ? bArr4.length : bArr.length - (this.f26845_V.length * i);
            if (length != 0) {
                addOneTo(this.f26845_V);
                this._engine.processBlock(this.f26845_V, 0, bArr4, 0);
