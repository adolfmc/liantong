 ...
 */
    ...
    public class CertId {
        ...
        CertId(PublicKey publicKey) {
            this.f26884id = PKCS12KeyStoreSpi.this.createSubjectKeyId(publicKey).getKeyIdentifier();
        }
        ...
        public boolean equals(Object obj) {
            ...
            if (obj instanceof CertId) {
                return Arrays.areEqual(this.f26884id, ((CertId) obj).f26884id);
            }
            return false;
        }

        public int hashCode() {
            return Arrays.hashCode(this.f26884id);
        }
    }
 ...
 */
    ...
    private void doStore(OutputStream outputStream, char[] cArr, boolean z) throws IOException {
        ...
        if (this.keys.size() == 0) {
            if (cArr == null) {
                ...
                if (z) {
                    pfx = new Pfx(new ContentInfo(PKCSObjectIdentifiers.data, new DEROctetString(new DERSequence(new ContentInfo(PKCSObjectIdentifiers.data, new DEROctetString(new DERSequence(aSN1EncodableVector).getEncoded()))).getEncoded())), null);
                    str = "DER";
                } else {
                    pfx = new Pfx(new ContentInfo(PKCSObjectIdentifiers.data, new BEROctetString(new BERSequence(new ContentInfo(PKCSObjectIdentifiers.data, new BEROctetString(new BERSequence(aSN1EncodableVector).getEncoded()))).getEncoded())), null);
                    str = "BER";
                }
    ...
    protected byte[] cryptData(boolean z, AlgorithmIdentifier algorithmIdentifier, char[] cArr, boolean z2, byte[] bArr) throws IOException {
        ...
        int i = z ? 1 : 2;
        if (algorithm.m366on(PKCSObjectIdentifiers.pkcs_12PbeIds)) {
    ...
    /* JADX WARN: Removed duplicated region for block: B:279:0x057c A[SYNTHETIC] */
    /* JADX WARN: Removed duplicated region for block: B:281:0x0574 A[SYNTHETIC] */
