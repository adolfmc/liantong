...
class FalconNIST {
    ...
    public FalconNIST(int i, int i2, SecureRandom secureRandom) {
        ...
        if (i == 9 || i == 8) {
            ...
            this.CRYPTO_SECRETKEYBYTES = (i5 * 2) + 1 + i5;
            this.CRYPTO_BYTES = 690;
        } else {
            i3 = this.f27137N;
            i4 = i3 * 7;
        }
        this.CRYPTO_SECRETKEYBYTES = ((i4 * 2) / 8) + 1 + i3;
        this.CRYPTO_BYTES = 690;
    }
    ...
    public byte[] crypto_sign(byte[] bArr, byte[] bArr2, int i, int i2, byte[] bArr3, int i3) {
        ...
        byte[] bArr10 = new byte[(this.CRYPTO_BYTES - 2) - i5];
        ...
        if (trim_i8_decode != 0) {
            int i6 = trim_i8_decode + 0;
            FalconCodec falconCodec2 = this.codec;
            int trim_i8_decode2 = falconCodec2.trim_i8_decode(bArr5, 0, this.LOGN, falconCodec2.max_fg_bits[this.LOGN], bArr3, i3 + i6, this.CRYPTO_SECRETKEYBYTES - i6);
            if (trim_i8_decode2 != 0) {
                int i7 = i6 + trim_i8_decode2;
                ...
                if (trim_i8_decode3 != 0) {
                    if (i7 + trim_i8_decode3 == this.CRYPTO_SECRETKEYBYTES - 1) {
                        if (falconVrfy.complete_private(bArr7, 0, bArr4, 0, bArr5, 0, bArr6, 0, this.LOGN, new short[this.f27137N * 2], 0) != 0) {
                            ...
                            shake256.inner_shake256_init();
                            shake256.inner_shake256_inject(bArr8, 0, bArr8.length);
                            shake256.i_shake256_flip();
                            falconSign.sign_dyn(sArr, 0, shake256, bArr4, 0, bArr5, 0, bArr6, 0, bArr7, 0, sArr2, 0, this.LOGN, new FalconFPR[this.f27137N * 10], 0);
                            int i8 = this.LOGN;
                            bArr10[0] = (byte) (i8 + 32);
                            int comp_encode = this.codec.comp_encode(bArr10, 1, bArr10.length - 1, sArr, 0, i8);
                            if (comp_encode != 0) {
                                int i9 = comp_encode + 1;
                                bArr[0] = (byte) (this.LOGN + 48);
                                System.arraycopy(bArr9, 0, bArr, 1, this.NONCELEN);
                                System.arraycopy(bArr10, 0, bArr, this.NONCELEN + 1, i9);
                                return Arrays.copyOfRange(bArr, 0, this.NONCELEN + 1 + i9);
                            }
    ...
    public byte[][] crypto_sign_keypair(byte[] bArr, int i, byte[] bArr2, int i2) {
        ...
        shake256.inner_shake256_init();
        shake256.inner_shake256_inject(bArr6, 0, bArr6.length);
        shake256.i_shake256_flip();
    ...
    public int crypto_sign_open(byte[] bArr, byte[] bArr2, byte[] bArr3, byte[] bArr4, int i) {
        ...
        if (length >= 1) {
            ...
            if (b == ((byte) (i4 + 32)) && this.codec.comp_decode(sArr3, 0, i4, bArr, 1, i2) == (i2 = length - 1)) {
                ...
                shake256.i_shake256_flip();
                falconCommon.hash_to_point_vartime(shake256, sArr2, 0, this.LOGN);
                return falconVrfy.verify_raw(sArr2, 0, sArr3, 0, sArr, 0, this.LOGN, new short[this.f27137N], 0) == 0 ? -1 : 0;
            }
            return -1;
        }
        return -1;
