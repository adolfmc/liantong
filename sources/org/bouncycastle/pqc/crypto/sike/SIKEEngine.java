    ...
    */
    ...
    public int crypto_kem_enc(byte[] bArr, byte[] bArr2, byte[] bArr3, SecureRandom secureRandom) {
        ...
        if (this.isCompressed) {
            ...
            byte[] bArr6 = new byte[this.params.FP2_ENCODED_BYTES];
            byte[] bArr7 = new byte[this.params.MSG_BYTES];
            bArr4 = new byte[this.params.CRYPTO_CIPHERTEXTBYTES + this.params.MSG_BYTES];
            byte[] bArr8 = new byte[this.params.MSG_BYTES];
            secureRandom.nextBytes(bArr8);
            System.arraycopy(bArr8, 0, bArr4, 0, this.params.MSG_BYTES);
            System.arraycopy(bArr3, 0, bArr4, this.params.MSG_BYTES, this.params.CRYPTO_PUBLICKEYBYTES);
            sHAKEDigest = new SHAKEDigest(256);
            sHAKEDigest.update(bArr4, 0, this.params.CRYPTO_PUBLICKEYBYTES + this.params.MSG_BYTES);
            sHAKEDigest.doFinal(bArr5, 0, this.params.SECRETKEY_B_BYTES);
            this.sidhCompressed.FormatPrivKey_B(bArr5);
            this.sidhCompressed.EphemeralKeyGeneration_B_extended(bArr5, bArr, 1);
            this.sidhCompressed.EphemeralSecretAgreement_B(bArr5, bArr3, bArr6);
            sHAKEDigest.update(bArr6, 0, this.params.FP2_ENCODED_BYTES);
            sHAKEDigest.doFinal(bArr7, 0, this.params.MSG_BYTES);
            for (int i = 0; i < this.params.MSG_BYTES; i++) {
                bArr[this.params.PARTIALLY_COMPRESSED_CHUNK_CT + i] = (byte) (bArr4[i] ^ bArr7[i]);
            }
        } else {
            ...
            bArr4 = new byte[this.params.CRYPTO_CIPHERTEXTBYTES + this.params.MSG_BYTES];
            byte[] bArr12 = new byte[this.params.MSG_BYTES];
            secureRandom.nextBytes(bArr12);
            System.arraycopy(bArr12, 0, bArr4, 0, this.params.MSG_BYTES);
            System.arraycopy(bArr3, 0, bArr4, this.params.MSG_BYTES, this.params.CRYPTO_PUBLICKEYBYTES);
            sHAKEDigest = new SHAKEDigest(256);
            sHAKEDigest.update(bArr4, 0, this.params.CRYPTO_PUBLICKEYBYTES + this.params.MSG_BYTES);
            ...
            for (int i3 = 0; i3 < this.params.MSG_BYTES; i3++) {
                bArr[this.params.CRYPTO_PUBLICKEYBYTES + i3] = (byte) (bArr4[i3] ^ bArr11[i3]);
            }
        }
        System.arraycopy(bArr, 0, bArr4, this.params.MSG_BYTES, this.params.CRYPTO_CIPHERTEXTBYTES);
        sHAKEDigest.update(bArr4, 0, this.params.CRYPTO_CIPHERTEXTBYTES + this.params.MSG_BYTES);
