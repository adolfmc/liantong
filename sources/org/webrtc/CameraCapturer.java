...
abstract class CameraCapturer implements CameraVideoCapturer {
    ...
    private final CameraSession.CreateSessionCallback createSessionCallback = new CameraSession.CreateSessionCallback() { // from class: org.webrtc.CameraCapturer.1
        ...
        public void onDone(CameraSession cameraSession) {
            ...
            Logging.m206d("CameraCapturer", "Create session done. Switch state: " + CameraCapturer.this.switchState);
            ...
            synchronized (CameraCapturer.this.stateLock) {
                ...
                if (CameraCapturer.this.switchState != SwitchState.IN_PROGRESS) {
                    if (CameraCapturer.this.switchState == SwitchState.PENDING) {
                        ...
                        CameraCapturer.this.switchCameraInternal(CameraCapturer.this.switchEventsHandler, str);
                    }
                } else {
                    CameraCapturer.this.switchState = SwitchState.IDLE;
        ...
        public void onFailure(CameraSession.FailureType failureType, String str) {
            ...
            synchronized (CameraCapturer.this.stateLock) {
                ...
                if (CameraCapturer.this.openAttemptsRemaining <= 0) {
                    Logging.m202w("CameraCapturer", "Opening camera failed, passing: " + str);
                    CameraCapturer.this.sessionOpening = false;
                    CameraCapturer.this.stateLock.notifyAll();
                    ...
                    Logging.m202w("CameraCapturer", "Opening camera failed, retry: " + str);
    ...
    private final CameraSession.Events cameraSessionEventsHandler = new CameraSession.Events() { // from class: org.webrtc.CameraCapturer.2
        ...
        public void onCameraOpening() {
            ...
            synchronized (CameraCapturer.this.stateLock) {
                if (CameraCapturer.this.currentSession == null) {
                    ...
                    Logging.m202w("CameraCapturer", "onCameraOpening while session was open.");
                }
            }
        }
        ...
        public void onCameraError(CameraSession cameraSession, String str) {
            ...
            synchronized (CameraCapturer.this.stateLock) {
                ...
                Logging.m202w("CameraCapturer", "onCameraError from another session: " + str);
            }
        }
        ...
        public void onCameraDisconnected(CameraSession cameraSession) {
            ...
            synchronized (CameraCapturer.this.stateLock) {
                ...
                Logging.m202w("CameraCapturer", "onCameraDisconnected from another session.");
            }
        }
        ...
        public void onCameraClosed(CameraSession cameraSession) {
            ...
            synchronized (CameraCapturer.this.stateLock) {
                if (cameraSession == CameraCapturer.this.currentSession || CameraCapturer.this.currentSession == null) {
                    ...
                    Logging.m206d("CameraCapturer", "onCameraClosed from another session.");
                }
            }
        }
        ...
        public void onFrameCaptured(CameraSession cameraSession, VideoFrame videoFrame) {
            ...
            synchronized (CameraCapturer.this.stateLock) {
                ...
                Logging.m202w("CameraCapturer", "onFrameCaptured from another session.");
            }
        }
    };
 ...
 */
    ...
    public void startCapture(int i, int i2, int i3) {
        Logging.m206d("CameraCapturer", "startCapture: " + i + "x" + i2 + "@" + i3);
        ...
        synchronized (this.stateLock) {
            ...
            Logging.m202w("CameraCapturer", "Session already open");
        }
    }
    ...
    public void stopCapture() {
        Logging.m206d("CameraCapturer", "Stop capture");
        synchronized (this.stateLock) {
            while (this.sessionOpening) {
                Logging.m206d("CameraCapturer", "Stop capture: Waiting for session to open");
                try {
                    this.stateLock.wait();
                } catch (InterruptedException unused) {
                    Logging.m202w("CameraCapturer", "Stop capture interrupted while waiting for the session to open.");
                    Thread.currentThread().interrupt();
                    return;
                }
            }
            if (this.currentSession != null) {
                Logging.m206d("CameraCapturer", "Stop capture: Nulling session");
                ...
                Logging.m206d("CameraCapturer", "Stop capture: No session open");
            }
        }
        Logging.m206d("CameraCapturer", "Stop capture done");
    }

    @Override // org.webrtc.VideoCapturer
    public void changeCaptureFormat(int i, int i2, int i3) {
        Logging.m206d("CameraCapturer", "changeCaptureFormat: " + i + "x" + i2 + "@" + i3);
        synchronized (this.stateLock) {
            stopCapture();
    ...
    @Override // org.webrtc.VideoCapturer
    public void dispose() {
        Logging.m206d("CameraCapturer", "dispose");
        stopCapture();
    }

    @Override // org.webrtc.CameraVideoCapturer
    public void switchCamera(final CameraVideoCapturer.CameraSwitchHandler cameraSwitchHandler) {
        Logging.m206d("CameraCapturer", "switchCamera");
    ...
    public void printStackTrace() {
        ...
        if (thread != null) {
            ...
            if (stackTrace.length > 0) {
                Logging.m206d("CameraCapturer", "CameraCapturer stack trace:");
                for (StackTraceElement stackTraceElement : stackTrace) {
                    Logging.m206d("CameraCapturer", stackTraceElement.toString());
                }
            }
        }
    }
    ...
    public void reportCameraSwitchError(String str, @Nullable CameraVideoCapturer.CameraSwitchHandler cameraSwitchHandler) {
        Logging.m205e("CameraCapturer", str);
    ...
    public void switchCameraInternal(@Nullable CameraVideoCapturer.CameraSwitchHandler cameraSwitchHandler, String str) {
        Logging.m206d("CameraCapturer", "switchCamera internal");
        ...
        synchronized (this.stateLock) {
            if (this.switchState != SwitchState.IDLE) {
                ...
                Logging.m206d("CameraCapturer", "switchCamera: Stopping session");
                this.cameraStatistics.release();
                this.cameraStatistics = null;
                final CameraSession cameraSession = this.currentSession;
                ...
                createSessionInternal(0);
                Logging.m206d("CameraCapturer", "switchCamera done");
            }
        }
    }
    ...
    public void checkIsOnCameraThread() {
        ...
        Logging.m205e("CameraCapturer", "Check is on camera thread failed.");
